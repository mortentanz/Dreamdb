/*-----------------------------------------------------------------------------------------------------------
  $Archive: /SQL.root/SQL/Demographics.Procedures.Queries/uspGetDSMothers.sql $
  $Revision: 1 $
  $Date: 12/18/06 15:46 $
  $Author: mls $

  Procedure   : Demographics.uspGetDSMothers
  
  Purpose     : Gets the number of mothers for a specified range of years. Intended for the population 
...
-------------------------------------------------------------------------------------------------------------*/
IF object_id('Demographics.uspGetDSMothers', 'P') IS NOT NULL
DROP PROCEDURE  Demographics.uspGetDSMothers
GO

CREATE PROCEDURE Demographics.uspGetDSMothers (
...
BEGIN
  ...
  SELECT OriginID, ChildGenderID, Age, [Year], Persons
