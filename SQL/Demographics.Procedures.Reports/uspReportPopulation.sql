/*-----------------------------------------------------------------------------------------------------------
  $Archive: /SQL.root/SQL/Demographics.Procedures.Reports/uspReportPopulation.sql $
  $Revision: 1 $
  $Date: 12/18/06 15:46 $
  $Author: mls $

  Procedure   : Demographics.uspReportPopulation
  
  Purpose     : Returns a result set presenting the population.

-----------------------------------------------------------------------------------------------------------*/
IF object_id('Demographics.uspReportPopulation', 'P') IS NOT NULL
DROP PROCEDURE Demographics.uspReportPopulation
GO

CREATE PROCEDURE Demographics.uspReportPopulation (
...
BEGIN
  ...
  SELECT * INTO #series
  FROM (
		SELECT OriginID, GenderID, Age, [Year], Persons FROM Demographics.vPopulationHistory
