/*-----------------------------------------------------------------------------------------------------------
  $Archive: /SQL.root/SQL/Demographics.Procedures.Reports/uspReportImmigration.sql $
  $Revision: 1 $
  $Date: 12/18/06 15:46 $
  $Author: mls $

  Procedure   : Demographics.uspReportImmigration
  
  Purpose     : Returns a result set presenting immigration.

-----------------------------------------------------------------------------------------------------------*/
IF object_id('Demographics.uspReportImmigration', 'P') IS NOT NULL
DROP PROCEDURE Demographics.uspReportImmigration
GO


CREATE PROCEDURE Demographics.uspReportImmigration (
...
BEGIN
  ...
  SELECT @f = min([Year]) FROM Demographics.Immigrants WHERE ProjectionID = @p;
